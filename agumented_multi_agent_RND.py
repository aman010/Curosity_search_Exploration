...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 23 15:19:42 2025
...
from feature_base_multiagent import curiosity_bonus
# ---------- USER CONFIG ----------
...
LOG_FILE = "agument_RND_multi_0.8-agent.csv"
if not os.path.exists(LOG_FILE):
    with open(LOG_FILE, "w", newline="") as f:
        write = csv.writer(f)
        write.writerow(["episode", "agent", "score", "action"])
        
def log_espisode(episode, agent, score):
    with open(LOG_FILE, "a", newline="") as f:
        write = csv.writer(f)
        write.writerow([episode, agent, score])


# ---------- ENV / MULTI-GAME RUNNER ----------
# ---------- Observation augmentation utilities (NumPy + OpenCV) ----------
import cv2
import random

# Augmentation hyperparameters (tweakable)
AUG_PROB = 0.8             # probability to apply augmentation to a sample
AUG_CROP_PCT = 0.06        # fraction to crop from edges for random crop/resize
AUG_SHIFT_PCT = 0.06       # fraction of width/height to randomly shift
AUG_BRIGHTNESS = 0.10      # brightness jitter (±)
AUG_CONTRAST = 0.10        # contrast jitter
AUG_NOISE_STD = 4.0        # gaussian noise std on [0,255] pixel scale
AUG_CUTOUT_PCT = 0.10      # cutout hole size fraction of min(H,W)
AUG_FLIP_PROB = 0.5        # horizontal flip prob


def _clamp_img(img):
    """Clamp float image to uint8 0..255"""
...
def random_crop_and_resize(img, crop_pct=AUG_CROP_PCT, out_size=(224,224)):
    ...
    resized = cv2.resize(cropped, (out_size[1], out_size[0]), interpolation=cv2.INTER_LINEAR)
    return resized
...
def random_cutout(img, cutout_pct=AUG_CUTOUT_PCT):
    ...
    img2 = img.copy()
    img2[y:y+cut_h, x:x+cut_w] = 127  # gray patch
    return img2
...
# RND-based curiosity (replaces pixel MSE curiosity)
# Running normalization for intrinsic (RND) rewards
_running_cur_mean = 0.0
_running_cur_var = 1.0
_cur_momentum = 0.99

# RND module (target = fixed random network, predictor = trainable)
class RNDEncoder(nn.Module):
    def __init__(self):
        ...
        # compute flattened conv size dynamically for OBS_SHAPE
        with torch.no_grad():
...
_rnd = RNDModel().to(DEVICE)
_rnd_opt = optim.Adam(_rnd.predictor.parameters(), lr=1e-4)

_recent_intrinsics = deque(maxlen=1000)
_INTRINSIC_NOISE_STD_THRESHOLD = 2.0


def rnd_intrinsic(obs_np):
    """Compute RND intrinsic bonus for a single observation (next state).
    Performs a single predictor update step (online) and returns the normalized
    intrinsic bonus (float, >=0).
    """
    global _running_cur_mean, _running_cur_var, _cur_momentum, _rnd_opt, _recent_intrinsics

    x = torch.from_numpy(obs_np).float().permute(2, 0, 1).unsqueeze(0).to(DEVICE) / 255.0

    with torch.no_grad():
        _, t_feats = _rnd(x)
    p_feats = _rnd.predictor(x)

    error = F.mse_loss(p_feats, t_feats, reduction='none').mean(dim=1).detach().cpu().numpy()
    err_val = float(error[0])

    # train predictor one step
    pred = _rnd.predictor(x)
    target = t_feats.detach()
    loss = F.mse_loss(pred, target)
    _rnd_opt.zero_grad()
    loss.backward()
    _rnd_opt.step()

    _running_cur_mean = _cur_momentum * _running_cur_mean + (1.0 - _cur_momentum) * err_val
    _running_cur_var = _cur_momentum * _running_cur_var + (1.0 - _cur_momentum) * ((err_val - _running_cur_mean) ** 2)
    std = np.sqrt(_running_cur_var) + 1e-8
    norm = (err_val - _running_cur_mean) / std
...
def train_two_agents():
    ...
    for update in range(1, TOTAL_UPDATES + 1):
        for step in range(STEPS_PER_UPDATE):
            ...
            for i in range(NUM_AGENTS):
                # augment observation for network input (do not send augmented to env)
                aug_obs = augment_observation(observations[i], out_size=(OBS_SHAPE[0], OBS_SHAPE[1]))
                a, logp, val = agents[i].get_action(aug_obs)
                actions[i] = a
                logps[i] = logp
                values[i] = val
            ...
            for i in range(NUM_AGENTS):
                ...
                # compute curiosity on augmented pair to avoid overfitting to pixel quirks
                aug_o = augment_observation(observations[i], out_size=(OBS_SHAPE[0], OBS_SHAPE[1]))
                aug_n = augment_observation(next_obs[i], out_size=(OBS_SHAPE[0], OBS_SHAPE[1]))
                R_cur = curiosity_bonus(aug_o, aug_n)
                ...
                log_espisode(update, i,episode_rewards)
